...
def setansiblehosts(apilisthosts,labuser): # updates the ansible hosts file
    ...
    for apiresult in apilisthosts:
        ...
        if vmname == 'captain':
            captstr = 'captain ansible_host=captain ansible_port=22 ansible_user=%s' % labuser
            hostsetup.set('lab-captain', captstr, None) 
        elif str(apiresult['name']).startswith('private'):
            privatestr = '%s ansible_host=%s ansible_port=22 ansible_user=%s' %  (vmname, vmname, labuser)
...
def main():
    ...
    setansiblehosts(current_inst,apiconfig['user'])  # write new hosts file
